...
const INITIAL_DATA_ZH = {
  ...
  locPin: "星巴克",
  visualScan:
    "穿着新的卡其色风衣，搭配深色牛仔裤。拿着一杯冰美式咖啡，感觉充满活力。",
...
const INITIAL_DATA_EN = {
  ...
  locPin: "Starbucks",
  visualScan:
    "Wearing the new khaki trench coat with dark jeans. Holding an iced americano, feeling alive.",
  humoral: { san: 80, phl: 60, cho: 90, mel: 30 },
  medievalRx:
    "Don't worry about things that haven't happened yet. Drink water, breathe.",
  consciousStream:
    "The table next to me is loud. Suddenly craving sukiyaki for dinner.",
...
export default function SoulReceiptGenerator() {
  ...
  const [data, setData] = useState(
    lang === "zh" ? INITIAL_DATA_ZH : INITIAL_DATA_EN
  ); // Initialize data based on global lang
  ...
  useEffect(() => {
    ...
    const progress = (
      ((nowTime - startOfYear) / (endOfYear - startOfYear)) *
      100
    ).toFixed(2);
    ...
    <div className="fixed inset-0 bg-gray-100 text-gray-900 flex flex-col md:flex-row md:justify-center overflow-hidden">
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto p-4 md:p-6 space-y-3 pb-24 md:pb-10 no-scrollbar">
          ...
          >
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <InputGroup
                label={t.envModLabel}
                value={data.envMod}
                onChange={(v) => setData({ ...data, envMod: v })}
              />
              <InputGroup
                label={t.locPinLabel}
                value={data.locPin}
                onChange={(v) => setData({ ...data, locPin: v })}
              />
            </div>
          </InputSection>
          <InputSection
          ...
          <InputSection
            title={t.humoralTitle}
            desc={t.humoralDesc}
            isOpen={expandedSection === "humoral"}
            onToggle={() => toggleSection("humoral")}
          >
            <div className="space-y-4 bg-gray-50 p-4 rounded-sm border border-gray-100">
              <SliderItem
                label={t.labelSan}
                value={data.humoral.san}
                onChange={(v) => updateHumoral("san", v)}
              />
              <SliderItem
                label={t.labelPhl}
                value={data.humoral.phl}
                onChange={(v) => updateHumoral("phl", v)}
              />
              <SliderItem
                label={t.labelCho}
                value={data.humoral.cho}
                onChange={(v) => updateHumoral("cho", v)}
              />
              <SliderItem
                label={t.labelMel}
          ...
          <InputSection
            title={t.consTitle}
            desc={t.consDesc}
            isOpen={expandedSection === "cons"}
            onToggle={() => toggleSection("cons")}
          >
            <div className="space-y-4">
              <InputGroup
                label={t.surfaceLabel}
                value={data.consciousStream}
                onChange={(v) => setData({ ...data, consciousStream: v })}
                isTextArea
              />
              <InputGroup
                label={t.deepLabel}
                value={data.deepLayer}
                onChange={(v) => setData({ ...data, deepLayer: v })}
                isTextArea
                className="border-l-4 border-gray-300 bg-gray-50/50"
              />
            </div>
          </InputSection>
          <InputSection
      ...
      <div className="flex-1 h-3 relative">
        ...
        >
          <div
            ...
            style={{
              backgroundImage:
                "repeating-linear-gradient(90deg, #000 0px, #000 3px, transparent 3px, transparent 4px)",
            }}
          ></div>
        </div>
      </div>
...
const ReceiptPreview = React.forwardRef(({ data, dateTime, text }, ref) => {
  ...
  const showSurface =
    data.consciousStream && data.consciousStream.trim().length > 0;
    ...
    >
      ...
      <div className="bg-white p-6 pb-8 flex-1 flex flex-col gap-4">
        ...
        <div className="text-center mb-2 leading-normal">
          <h1
            className="font-bold mb-1 tracking-[0.1em] text-black"
        ...
        <div>
          <div className="grid grid-cols-2 gap-y-1 text-sm font-bold w-full leading-relaxed">
            <div className="pr-1 whitespace-nowrap overflow-hidden">
              {text.envModLabel.toUpperCase()}:{" "}
              <span className="font-normal">{data.envMod}</span>
            </div>
            <div className="pl-1 text-right whitespace-nowrap overflow-hidden">
              {text.locPinLabel.toUpperCase()}:{" "}
              <span className="font-normal">{data.locPin}</span>
            </div>
            <div className="pr-1 whitespace-nowrap">
              DATE: <span className="font-normal">{dateTime.date}</span>
            </div>
            <div className="pl-1 text-right whitespace-nowrap">
              TIME: <span className="font-normal">{dateTime.time}</span>{" "}
              <span className="text-[10px] font-normal align-middle">
                {dateTime.zone}
              </span>
            </div>
          </div>
        ...
        {showVisual && (
          <div className="mt-4">
            <div className="bg-black text-white inline-flex items-center px-2 py-1 text-xs font-bold mb-2 leading-none">
              {text.visualLabel}
            </div>
            <div className="border-l-4 border-black pl-3 py-1 text-sm leading-relaxed text-gray-800 break-words whitespace-pre-wrap">
              {data.visualScan}
            </div>
          </div>
        )}
        ...
        <div className="border border-black p-3 mt-2">
          <div className="text-center text-xs font-bold mb-3 uppercase leading-relaxed">
            -- {text.humoralTitle} --
          </div>
          <div className="flex h-6 border border-black mb-3 w-full">
            <div
              style={{ width: `${getWidth(san)}%` }}
              className="border-r border-black relative h-full"
            >
            ...
            <div
              style={{ width: `${getWidth(phl)}%` }}
              className="border-r border-black h-full"
            ></div>
            <div
              style={{ width: `${getWidth(cho)}%` }}
              className="border-r border-black h-full"
            >
              <svg width="100%" height="100%">
                <defs>
            ...
            {showSurface && (
              <>
                <div className="text-xs text-gray-500 mb-1 leading-relaxed">
                  {">>>"} {text.surfaceLabel.toUpperCase()}
                </div>
                <div className="border-b border-black mb-3"></div>
                <p className="text-sm italic text-gray-700 mb-4 px-1 break-words whitespace-pre-wrap leading-relaxed">
                  "{data.consciousStream}"
                </p>
              </>
            )}
            {showDeep && (
              <>
                <div className="text-right text-xs text-gray-500 mb-1 leading-relaxed">
                  {text.deepLabel.toUpperCase()} {"<<<"}
                </div>
                <div className="border-t border-black mb-1"></div>
                <div className="bg-gray-100 border-l-4 border-black p-3 text-sm font-medium break-words whitespace-pre-wrap leading-relaxed">
                  {data.deepLayer}
                </div>
              </>
            )}
        ...
        {showTasks && (
          <div className="mt-4">
            <div className="flex items-center gap-2 mb-3">
              ...
              <h3 className="font-bold text-sm tracking-widest leading-relaxed">
                {text.tasksTitle.toUpperCase()}
              </h3>
              <div className="h-px bg-black flex-1 border-t border-dashed border-gray-400"></div>
            </div>
        ...
        <div className="mt-1 flex flex-col items-center">
          ...
          <div className="text-[10px] tracking-widest text-gray-500 uppercase mt-2 leading-relaxed">
            {text.endTransaction}
          </div>
        </div>
      </div>
