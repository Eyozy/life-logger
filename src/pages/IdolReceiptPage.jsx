...
import { ChevronLeft, Download, Loader2, Globe, ChevronDown, Heart, Plus, Trash2, Zap, Smile, Wallet, Sparkles, Eye } from 'lucide-react';
...
const TEXT = {
  zh: {
    title: "追星日志",
    subtitle: "// 追星日志小票",
    idolInfo: "本命信息 (Idol Info)",
    supportList: "应援/氪金 (Support)",
    vitalSigns: "生命体征 (Vital Signs)",
    thoughts: "发疯时刻 (Thoughts)",
...
export default function IdolReceiptPage() {
  ...
  const [data, setData] = useState({
    ...
    scream: 5,
    highlight: "Hanni 今天的双马尾造型简直是神！！！我的天哪怎么会有这么可爱的人类...",
  ...
  const addItem = () => setData({ ...data, items: [...data.items, { name: "", price: "" }] });
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      link.download = `IDOL_LOG_${Date.now()}.png`;
      link.href = dataUrl;
      link.click();
    } catch (error) {
      alert("生成失败");
    ...
    <div className="fixed inset-0 bg-gray-50 text-gray-900 font-mono flex flex-col md:flex-row overflow-hidden">
      ...
      <div className={`w-full md:w-1/2 flex flex-col h-full bg-white md:border-r border-gray-200 z-10 transition-transform duration-300 ${mobileTab === 'edit' ? 'translate-x-0' : '-translate-x-full md:translate-x-0'} absolute md:relative`}>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3 pb-24 md:pb-10 no-scrollbar">
          ...
          <InputSection title={t.idolInfo} isOpen={expandedSection === 'idol'} onToggle={() => toggleSection('idol')}>
            <InputGroup label={t.labelName} value={data.name} onChange={v => setData({...data, name: v})} />
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-[10px] font-bold text-gray-400 mb-1.5 uppercase">{t.labelActivity}</label>
                <select value={data.activity} onChange={e => setData({...data, activity: e.target.value})} className="w-full bg-gray-50 border border-gray-200 p-2.5 text-sm focus:border-black outline-none rounded-sm">
              ...
              <div>
                <label className="block text-[10px] font-bold text-gray-400 mb-1.5 uppercase">{t.labelIdentity}</label>
                <select value={data.identity} onChange={e => setData({...data, identity: e.target.value})} className="w-full bg-gray-50 border border-gray-200 p-2.5 text-sm focus:border-black outline-none rounded-sm">
          ...
          <InputSection title={t.supportList} isOpen={expandedSection === 'items'} onToggle={() => toggleSection('items')}>
            {data.items.map((item, i) => (
              <div key={i} className="flex gap-2 mb-2">
                <input value={item.name} onChange={e => updateItem(i, 'name', e.target.value)} className="flex-1 bg-gray-50 border border-gray-200 p-2 text-sm rounded-sm focus:border-black outline-none" placeholder={t.labelItem} />
                <input value={item.price} onChange={e => updateItem(i, 'price', e.target.value)} className="w-16 bg-gray-50 border border-gray-200 p-2 text-sm rounded-sm focus:border-black outline-none text-right" placeholder={t.labelCost} />
                <button onClick={() => removeItem(i)} className="text-gray-300 hover:text-red-500"><Trash2 size={16} /></button>
              </div>
            ))}
            <button onClick={addItem} className="mt-2 text-xs font-bold flex items-center gap-1 text-pink-600 hover:text-pink-700"><Plus size={14} /> ADD ITEM</button>
          </InputSection>

          <InputSection title={t.vitalSigns} isOpen={expandedSection === 'stats'} onToggle={() => toggleSection('stats')}>
            <div className="space-y-4 bg-gray-50 p-4 rounded-sm border border-gray-100">
              <SliderItem label={t.labelDopamine} value={data.stats.dopamine} onChange={v => setData({...data, stats: {...data.stats, dopamine: v}})} icon={Smile} />
              <SliderItem label={t.labelSanity} value={data.stats.sanity} onChange={v => setData({...data, stats: {...data.stats, sanity: v}})} icon={Brain} />
              <SliderItem label={t.labelWallet} value={data.stats.wallet} onChange={v => setData({...data, stats: {...data.stats, wallet: v}})} icon={Wallet} />
              
              <div className="border-t border-gray-200 pt-4 mt-2">
                 <div className="flex items-center justify-between">
                   <div className="flex items-center gap-2 text-gray-500">
                     <Zap size={14} />
                     <span className="text-[10px] font-bold uppercase">{t.labelScream}</span>
                   </div>
                   <div className="flex gap-1">
                      {[1,2,3,4,5].map(s => (
                        <button key={s} onClick={() => setData({...data, scream: s})} className={s <= data.scream ? "text-pink-500" : "text-gray-300"}><Heart size={16} fill={s <= data.scream ? "currentColor" : "none"} /></button>
                      ))}
                   </div>
                 </div>
              </div>
          ...
          <InputSection title={t.thoughts} isOpen={expandedSection === 'thoughts'} onToggle={() => toggleSection('thoughts')}>
            <InputGroup label={t.labelHighlight} value={data.highlight} onChange={v => setData({...data, highlight: v})} isTextArea />
            <InputGroup label={t.labelPromise} value={data.promise} onChange={v => setData({...data, promise: v})} isTextArea />
          </InputSection>
...
const IdolPreview = React.forwardRef(({ data, text }, ref) => {
  const total = data.items.reduce((sum, item) => sum + (parseFloat(item.price) || 0), 0).toFixed(2);
  ...
  return (
    <div ref={ref} className="w-[380px] relative">
      ...
      <div className="mb-4 text-center">
        <h1 className="text-3xl font-black tracking-wider mb-1 text-black">{text.title}</h1>
      ...
      <div className="grid grid-cols-[max-content_1fr] gap-x-4 gap-y-1 text-xs font-bold uppercase mb-4 leading-relaxed">
        <div className="text-gray-500">{text.labelDate}:</div><div className="text-black">{data.date}</div>
        <div className="text-gray-500">{text.labelName}:</div><div className="text-black truncate">{data.name}</div>
        <div className="text-gray-500">{text.labelActivity}:</div><div className="text-black truncate">{text.activities[data.activity]}</div>
        <div className="text-gray-500">{text.labelIdentity}:</div><div className="text-black truncate">{text.identities[data.identity]}</div>
      </div>

      {/* Support List */}
      <div className="mb-4">
         <div className="border-b border-black pb-1 mb-2 flex justify-between">
            <span className="text-[10px] font-bold uppercase text-gray-600">{text.supportList}</span>
         </div>
        <div className="space-y-1.5">
          {data.items.map((item, i) => (
            <div key={i} className="flex justify-between text-sm items-start leading-tight group">
              <span className="max-w-[240px] text-black">{item.name || "---"}</span>
              <span className="font-bold font-mono">{item.price || "0.00"}</span>
            </div>
          ))}
        </div>
        <div className="flex justify-between text-sm font-black border-t-2 border-black pt-2 mt-4 text-black">
      ...
      <div className="border-2 border-black p-4 mb-4 font-mono text-xs">
         <div className="text-center text-[10px] font-bold mb-3 uppercase text-black border-b border-dashed border-black pb-1">-- {text.vitalSigns} --</div>
         
         <div className="space-y-3">
            <div className="flex items-center gap-2">
               <span className="w-24 text-[10px] font-bold uppercase">{text.labelDopamine}</span>
               <Bar val={data.stats.dopamine} />
               <span className="w-8 text-right font-bold">{data.stats.dopamine}%</span>
            </div>
            <div className="flex items-center gap-2">
               <span className="w-24 text-[10px] font-bold uppercase">{text.labelSanity}</span>
               <Bar val={data.stats.sanity} />
               <span className="w-8 text-right font-bold">{data.stats.sanity}%</span>
            </div>
            <div className="flex items-center gap-2">
               <span className="w-24 text-[10px] font-bold uppercase">{text.labelWallet}</span>
               <Bar val={data.stats.wallet} />
               <span className="w-8 text-right font-bold">{data.stats.wallet}%</span>
            </div>
            
            <div className="flex justify-between items-center pt-2 border-t border-gray-100 mt-2">
               <span className="text-[10px] font-bold uppercase">{text.labelScream}</span>
               <div className="flex gap-1">
                  {[1,2,3,4,5].map(s => (
                     <span key={s} className="text-sm">{s <= data.scream ? "★" : "☆"}</span>
                  ))}
               </div>
            </div>
         </div>
      ...
      {data.highlight && data.highlight.trim() && (
        <div className="mb-4">
          <div className="text-[10px] font-bold uppercase text-gray-500 border-b border-black pb-1 mb-2">{text.labelHighlight}</div>
          <div className="relative pl-3 border-l-4 border-black">
            <p className="text-sm italic font-serif leading-relaxed text-black text-justify whitespace-pre-wrap">"{data.highlight}"</p>
          </div>
        </div>
      )}
      ...
      {data.promise && data.promise.trim() && (
        <div className="mb-6 flex-1">
          <div className="text-[10px] font-bold uppercase text-gray-500 border-b border-black pb-1 mb-2">{text.labelPromise}</div>
          <p className="text-xs font-mono leading-relaxed text-gray-800 whitespace-pre-wrap">{data.promise}</p>
        </div>
      )}

      {/* Footer */}
      <div className="mt-auto pt-4 flex flex-col items-center">
         <div className="text-center font-bold text-black mb-2">( &gt; ‿ ♥ )</div>
