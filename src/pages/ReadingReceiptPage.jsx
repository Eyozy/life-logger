...
import { ChevronLeft, Download, Loader2, ChevronDown, Globe, BookOpen, Bookmark, Feather, Coffee, Eye } from 'lucide-react';
...
export default function ReadingReceiptPage() {
  ...
  const [data, setData] = useState({
    ...
    rating: 5,
    quote: "知识可以传授，但智慧不能。人们可以找到智慧，在生活中实行智慧，以智慧自强，以智慧创造奇迹，但人们不可能传授智慧。",
    review: "一本关于自我探索的流浪之书。读完内心非常平静，仿佛也听到了河水的声音。黑塞的文字如诗般优美，值得反复阅读。"
  });
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      link.download = `READING_LOG_${Date.now()}.png`;
      link.href = dataUrl;
      link.click();
    } catch (error) {
      alert("生成失败");
    ...
    <div className="fixed inset-0 bg-gray-50 text-gray-900 font-mono flex flex-col md:flex-row overflow-hidden">
      ...
      <div className={`w-full md:w-1/2 flex flex-col h-full bg-white md:border-r border-gray-200 z-10 transition-transform duration-300 ${mobileTab === 'edit' ? 'translate-x-0' : '-translate-x-full md:translate-x-0'} absolute md:relative`}>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3 pb-24 md:pb-10 no-scrollbar">
          ...
          <InputSection title={t.bookInfo} isOpen={expandedSection === 'book'} onToggle={() => toggleSection('book')}>
            <InputGroup label={t.labelTitle} value={data.title} onChange={v => setData({...data, title: v})} />
            <InputGroup label={t.labelAuthor} value={data.author} onChange={v => setData({...data, author: v})} />
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                <label className="block text-[10px] font-bold text-gray-400 mb-1.5 uppercase">{t.labelFormat}</label>
                <select value={data.format} onChange={e => setData({...data, format: e.target.value})} className="w-full bg-gray-50 border border-gray-200 p-2.5 text-sm focus:border-black outline-none rounded-sm">
            ...
            <div className="grid grid-cols-2 gap-4">
               <InputGroup label={t.labelPages} value={data.pages} onChange={v => setData({...data, pages: v})} />
            ...
            <div className="space-y-4 bg-gray-50 p-4 rounded-sm border border-gray-100">
              <div className="flex justify-between items-center pb-2 border-b border-gray-200">
                 ...
                 <div className="flex gap-1">
                    {[1,2,3,4,5].map(s => (
                      <button key={s} onClick={() => setData({...data, rating: s})} className={s <= data.rating ? "text-black" : "text-gray-300"}>
                        {s <= data.rating ? "★" : "☆"}
                      </button>
                    ))}
                 </div>
              </div>
              <SliderItem label={t.labelImmersion} value={data.metrics.immersion} onChange={v => setData({...data, metrics: {...data.metrics, immersion: v}})} />
              <SliderItem label={t.labelIntellect} value={data.metrics.intellect} onChange={v => setData({...data, metrics: {...data.metrics, intellect: v}})} />
              <SliderItem label={t.labelEmotion} value={data.metrics.emotion} onChange={v => setData({...data, metrics: {...data.metrics, emotion: v}})} />
              <SliderItem label={t.labelWriting} value={data.metrics.writing} onChange={v => setData({...data, metrics: {...data.metrics, writing: v}})} />
            </div>
          </InputSection>

          <InputSection title={t.thoughts} isOpen={expandedSection === 'thoughts'} onToggle={() => toggleSection('thoughts')}>
            <InputGroup label={t.labelQuote} value={data.quote} onChange={v => setData({...data, quote: v})} isTextArea />
            <InputGroup label={t.labelReview} value={data.review} onChange={v => setData({...data, review: v})} isTextArea />
          </InputSection>
  ...
  return (
    <div ref={ref} className="w-[380px] relative">
      ...
      <div className="text-center mb-4">
        <h1 className="text-3xl font-bold tracking-wider mb-1 text-black uppercase">THE_LIBRARY</h1>
        <div className="text-[10px] font-bold text-gray-500 tracking-widest">{text.labelHeader}</div>
      </div>
      ...
      {data.title && data.title.trim() && (
        <div className="mb-4">
          <div className="flex justify-between text-xs font-bold mb-1">
             <span className="text-gray-500 uppercase">{text.labelTitle}:</span>
             <span className="text-black max-w-[240px] text-right">{data.title}</span>
          </div>
          <div className="flex justify-between text-xs font-bold mb-1">
             <span className="text-gray-500 uppercase">{text.labelAuthor}:</span>
          ...
          <div className="flex gap-2 text-[10px] font-bold uppercase border-b border-black pb-3">
             <span className="border border-black px-1.5 py-0.5">{text.genres[data.genre]}</span>
             <span className="bg-black text-white px-1.5 py-0.5">{text.formats[data.format]}</span>
             <span className="border border-black px-1.5 py-0.5">{data.pages}</span>
          </div>
        </div>
      )}
      ...
      {/* Metrics */}
      <div className="mb-6">
         <div className="text-[10px] font-bold uppercase mb-2 text-gray-500 text-center">{text.labelAnalysis}</div>
         <div className="space-y-2 text-xs font-bold">
            <div className="flex items-center gap-2">
               <span className="w-20 uppercase text-[9px]">{text.labelImmersion}</span>
               <Bar val={data.metrics.immersion} />
               <span className="w-6 text-right">{data.metrics.immersion}</span>
            </div>
            <div className="flex items-center gap-2">
               <span className="w-20 uppercase text-[9px]">{text.labelIntellect}</span>
               <Bar val={data.metrics.intellect} />
               <span className="w-6 text-right">{data.metrics.intellect}</span>
            </div>
            <div className="flex items-center gap-2">
               <span className="w-20 uppercase text-[9px]">{text.labelEmotion}</span>
               <Bar val={data.metrics.emotion} />
               <span className="w-6 text-right">{data.metrics.emotion}</span>
            </div>
            <div className="flex items-center gap-2">
               <span className="w-20 uppercase text-[9px]">{text.labelWriting}</span>
      ...
      {data.quote && (
        <div className="mb-6">
          <div className="text-[10px] font-bold uppercase text-gray-500 mb-1">{text.labelQuote}</div>
          <div className="border-l-4 border-black pl-3 py-1">
             <p className="text-sm italic font-serif leading-relaxed whitespace-pre-wrap">"{data.quote}"</p>
          </div>
        </div>
      )}
      ...
      {data.review && (
        <div className="mb-6 flex-1">
          <div className="text-[10px] font-bold uppercase text-gray-500 mb-1">{text.labelReview}</div>
      ...
      <div className="mt-auto pt-6 flex flex-col items-center">
         ...
         <div className="flex h-10 w-full justify-center items-stretch gap-0.5 overflow-hidden">
            {[...Array(40)].map((_, i) => <div key={i} className="bg-black" style={{width: Math.random() > 0.5 ? '2px' : '4px'}}></div>)}
         </div>
         <div className="text-[10px] font-mono mt-1">{Date.now().toString().slice(-10)}</div>
      </div>
