...
import { ChevronLeft, Download, Loader2, ChevronDown, Globe, Music, MapPin, Calendar, Headphones, Eye } from 'lucide-react';
...
const RatingSlider = ({ label, value, onChange }) => (
  <div className="flex items-center gap-3">
    ...
    />
    <span className="w-8 font-mono text-xs font-bold text-right text-gray-900">{value}%</span>
  </div>
);
...
export default function MusicConcertReceiptPage() {
  ...
  const [data, setData] = useState({
    ...
    thoughts: "现场氛围超级棒，音质完美，舞美很震撼。最感动的是全场一起合唱经典老歌的时刻。杰伦的状态很好，还是那么有魅力。"
  });
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      link.download = `MUSIC_CONCERT_${Date.now()}.png`;
      link.href = dataUrl;
      link.click();
    } catch (error) {
      alert("生成失败");
    ...
    <div className="fixed inset-0 flex flex-col overflow-hidden font-mono text-gray-900 bg-gray-50 md:flex-row">
      ...
      <div className={`w-full md:w-1/2 flex flex-col h-full bg-white md:border-r border-gray-200 z-10 transition-transform duration-300 ${mobileTab === 'edit' ? 'translate-x-0' : '-translate-x-full md:translate-x-0'} absolute md:relative`}>
        ...
        <div className="flex-1 p-4 pb-24 space-y-3 overflow-y-auto md:pb-10 no-scrollbar">
          ...
          <InputSection title={t.concertInfo} isOpen={expandedSection === 'concert'} onToggle={() => toggleSection('concert')}>
            <InputGroup label={t.labelArtist} value={data.artist} onChange={v => setData({...data, artist: v})} />
            <InputGroup label={t.labelTour} value={data.tour} onChange={v => setData({...data, tour: v})} />
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-[10px] font-bold text-gray-400 mb-1.5 uppercase">{t.labelVenue}</label>
                <select value={data.venue} onChange={e => setData({...data, venue: e.target.value})} className="w-full bg-gray-50 border border-gray-200 p-2.5 text-sm focus:border-black outline-none rounded-sm">
                  {Object.entries(t.venues).map(([key, label]) => (
                    <option key={key} value={key}>{label}</option>
                  ))}
                </select>
              </div>
              <InputGroup label="场地名称" value={data.venueName} onChange={v => setData({...data, venueName: v})} />
            </div>
            <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-[10px] font-bold text-gray-400 mb-1.5 uppercase">音乐类型</label>
          ...
          <InputSection title={t.experience} isOpen={expandedSection === 'experience'} onToggle={() => toggleSection('experience')}>
            <div className="p-4 space-y-4 border border-gray-100 rounded-sm bg-gray-50">
              <RatingSlider label={t.labelAcoustic} value={data.experience.acoustic} onChange={v => setData({...data, experience: {...data.experience, acoustic: v}})} />
              ...
              <RatingSlider label={t.labelAtmosphere} value={data.experience.atmosphere} onChange={v => setData({...data, experience: {...data.experience, atmosphere: v}})} />
              <RatingSlider label={t.labelEnergy} value={data.experience.energy} onChange={v => setData({...data, experience: {...data.experience, energy: v}})} />
              <RatingSlider label={t.labelVocals} value={data.experience.vocals} onChange={v => setData({...data, experience: {...data.experience, vocals: v}})} />
            </div>
          </InputSection>

          <InputSection title={t.setlist} isOpen={expandedSection === 'setlist'} onToggle={() => toggleSection('setlist')}>
            <InputGroup label={t.labelFavorite} value={data.favoriteSong} onChange={v => setData({...data, favoriteSong: v})} isTextArea />
            <InputGroup label={t.labelSurprise} value={data.surpriseMoment} onChange={v => setData({...data, surpriseMoment: v})} isTextArea />
          </InputSection>
...
const MusicConcertPreview = React.forwardRef(({ data, text }, ref) => {
  const avgExperience = Math.round((data.experience.acoustic + data.experience.visual + data.experience.atmosphere + data.experience.energy + data.experience.vocals) / 5);
  ...
  return (
    <div ref={ref} className="w-[380px] relative">
      ...
      <div className="mb-4 text-center">
        <h1 className="mb-1 text-3xl font-bold tracking-wider text-black">{text.title}</h1>
        <div className="text-[10px] font-bold text-gray-500 tracking-widest">CONCERT EXPERIENCE</div>
      </div>
      ...
      {data.artist && data.artist.trim() && (
        <div className="mb-6 text-center">
          <div className="text-[10px] font-bold uppercase text-gray-500 mb-1">{text.labelArtist}</div>
          <h2 className="mb-2 text-xl font-bold leading-snug">{data.artist}</h2>
          ...
          <div className="flex gap-2 text-[10px] font-bold uppercase mb-2 justify-center">
            <span className="px-2 border border-black">{text.venues[data.venue]}</span>
            <span className="px-2 text-white bg-black">{text.genres[data.genre]}</span>
          </div>
        </div>
      )}
      ...
      <div className="p-4 mb-6 border border-black">
        <div className="grid grid-cols-2 gap-2 text-[10px] font-bold uppercase mb-2">
          <div><MapPin className="inline w-3 h-3 mr-1" /> {text.labelVenue}:</div>
          <div>{data.venueName}</div>
          <div><Calendar className="inline w-3 h-3 mr-1" /> {text.labelDate}:</div>
          <div>{data.date}</div>
          <div><Headphones className="inline w-3 h-3 mr-1" /> {text.labelDuration}:</div>
          <div>{data.duration}</div>
          <div className="col-span-2"><span className="text-gray-500">{text.labelSeat}:</span> {data.seat}</div>
        </div>
      </div>

      {/* Experience Ratings */}
      <div className="mb-6">
        <div className="text-[10px] font-bold uppercase text-gray-500 border-b border-black pb-1 mb-2">Live Experience</div>
        <div className="flex items-center gap-4 mb-4">
          ...
          <div className="flex-1 space-y-1">
            ...
            <div className="relative h-2 bg-gray-200 border border-black">
              <div className="h-full bg-black" style={{ width: `${data.experience.acoustic}%` }}></div>
            </div>
            <div className="flex justify-between text-[10px] font-bold uppercase">
            ...
            <div className="relative h-2 bg-gray-200 border border-black">
              <div className="h-full bg-black" style={{ width: `${data.experience.visual}%` }}></div>
            </div>
            <div className="flex justify-between text-[10px] font-bold uppercase">
            ...
            <div className="relative h-2 bg-gray-200 border border-black">
              <div className="h-full bg-black" style={{ width: `${data.experience.atmosphere}%` }}></div>
            </div>
            <div className="flex justify-between text-[10px] font-bold uppercase">
            ...
            <div className="relative h-2 bg-gray-200 border border-black">
              <div className="h-full bg-gradient-to-r from-yellow-400 to-red-500" style={{ width: `${data.experience.energy}%` }}></div>
            </div>
            <div className="flex justify-between text-[10px] font-bold uppercase">
            ...
            <div className="relative h-2 bg-gray-200 border border-black">
              <div className="h-full bg-black" style={{ width: `${data.experience.vocals}%` }}></div>
            </div>
          </div>
        </div>
      </div>
      ...
      {data.favoriteSong && data.favoriteSong.trim() && (
        <div className="mb-6">
          <div className="text-[10px] font-bold uppercase text-gray-500 border-b border-black pb-1 mb-2">Highlight Moment</div>
          <div className="relative p-4 border-l-4 border-black bg-gray-50">
            <Music className="absolute w-6 h-6 text-black top-2 right-2" />
            <p className="text-sm font-bold leading-relaxed whitespace-pre-wrap">"{data.favoriteSong}"</p>
          </div>
        </div>
      )}
      ...
      {data.surpriseMoment && data.surpriseMoment.trim() && (
        <div className="mb-6">
          <div className="text-[10px] font-bold uppercase text-gray-500 border-b border-black pb-1 mb-2">Surprise Moment</div>
          <div className="relative p-4 border-l-4 border-black bg-gray-50">
            <p className="text-sm italic leading-relaxed text-justify text-gray-800 whitespace-pre-wrap">"{data.surpriseMoment}"</p>
          </div>
        </div>
      )}
      ...
      {data.thoughts && data.thoughts.trim() && (
        <div className="flex-1 mb-6">
          ...
          <p className="pb-2 text-sm leading-relaxed whitespace-pre-wrap border-b border-gray-300">{data.thoughts}</p>
        </div>
      )}

      {/* Footer */}
      <div className="flex flex-col items-center pt-6 mt-auto">
        <div className="text-[10px] font-bold uppercase tracking-widest mb-2 text-center">Thank You For Coming</div>
