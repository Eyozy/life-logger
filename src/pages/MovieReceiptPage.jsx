...
const RatingSlider = ({ label, value, onChange }) => (
  <div className="flex items-center gap-3">
    ...
    <div className="flex items-center flex-1 gap-1" role="radiogroup" aria-label={label}>
      {[1, 2, 3, 4, 5].map((star) => (
        <button
          key={star}
          type="button"
...
export default function MovieReceiptPage() {
  ...
  const [data, setData] = useState({
    ...
    personalThoughts:
      "相比第一部，这一部更深入地探讨了政治和宗教主题，但也失去了一些角色发展的细腻度。整体依然是一部史诗级的科幻巨作。",
  });

  const [expandedSection, setExpandedSection] = useState("movie");
  const receiptRef = useRef(null);
  const { isDownloading, handleDownload, error } = useDownloadReceipt('MOVIE_LOG');
      ...
      >
        ...
        <div className="flex-1 p-4 pb-24 space-y-3 overflow-y-auto md:pb-10">
          ...
          >
            ...
            />
            <InputGroup
              label={t.labelDirector}
              value={data.director}
              onChange={(v) => setData({ ...data, director: v })}
            />
            ...
            <InputGroup
              label={t.labelCinema}
              value={data.cinema}
              onChange={(v) => setData({ ...data, cinema: v })}
            />
            <InputGroup
              label={t.labelSeat}
              value={data.seat}
          ...
          >
            <div className="p-4 space-y-4 border border-gray-100 rounded-sm bg-gray-50">
              ...
              <RatingSlider
                label={t.labelStory}
                value={data.experience.story}
                onChange={(v) =>
                  setData({
                    ...data,
                    experience: { ...data.experience, story: v },
                  })
                }
              />
              <RatingSlider
                label={t.labelActing}
                value={data.experience.acting}
                onChange={(v) =>
                  setData({
                    ...data,
                    experience: { ...data.experience, acting: v },
                  })
                }
              />
              <RatingSlider
                label={t.labelMusic}
                value={data.experience.music}
                onChange={(v) =>
                  setData({
                    ...data,
                    experience: { ...data.experience, music: v },
                  })
                }
              />
              ...
              <div className="pt-4 mt-2 border-t border-gray-200">
                <RatingSlider
                  ...
                  value={data.emotion}
                  onChange={(v) => setData({ ...data, emotion: v })}
                />
              </div>
              <RatingSlider
                label={t.labelImmersion}
                value={data.immersion}
                onChange={(v) => setData({ ...data, immersion: v })}
              />
            </div>
          </InputSection>
          ...
          >
            <InputGroup
              ...
              onChange={(v) => setData({ ...data, favoriteScene: v })}
              isTextArea
            />
            <InputGroup
              label={t.labelThoughts}
              value={data.personalThoughts}
              onChange={(v) => setData({ ...data, personalThoughts: v })}
              isTextArea
            />
          </InputSection>
        </div>
...
const MoviePreview = React.forwardRef(({ data, text }, ref) => {
  const avgRating = Math.round(
    (data.experience.visual +
      ...
      data.experience.acting +
      data.experience.music) /
      4
  );
      ...
      <div className="min-h-[600px] bg-white p-6 pb-8 font-receipt tracking-receipt flex flex-col">
        ...
        {data.title && data.title.trim() && (
          <div className="mb-6">
            ...
            <p className="text-xs font-bold text-gray-600">
              {text.labelDirector}: {data.director}
            </p>
          </div>
        )}
        ...
        <div className="p-4 mb-6 border border-black">
          <div className="grid grid-cols-2 gap-2 text-[10px] font-bold uppercase mb-2">
            <div>
              <span className="text-gray-500">{text.labelCinema}:</span>{" "}
              {data.cinema}
            </div>
            <div>
            ...
            <div className="col-span-2">
              <span className="text-gray-500">{text.labelDate}:</span>{" "}
              {data.date}
            </div>
          </div>
        </div>
            ...
            <div>
              ...
              <div className="relative h-2 bg-gray-200 border border-black">
                <div
                  ...
                  style={{ width: `${(data.emotion / 5) * 100}%` }}
                ></div>
              </div>
            </div>
            <div>
              ...
              <div className="relative h-2 bg-gray-200 border border-black">
                <div
                  className="h-full bg-black"
                  style={{ width: `${(data.immersion / 5) * 100}%` }}
                ></div>
              </div>
            </div>
          </div>
        ...
        {data.favoriteScene && data.favoriteScene.trim() && (
          <div className="mb-6">
            ...
            <div className="relative p-4 border-l-4 border-black bg-gray-50">
              <p className="text-sm italic leading-loose text-justify text-gray-800 whitespace-pre-wrap">
                "{data.favoriteScene}"
              </p>
            </div>
          </div>
