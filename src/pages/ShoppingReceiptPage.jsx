...
import { ChevronLeft, Download, Loader2, Globe, Eye, Edit3, Plus, Trash2 } from 'lucide-react';
...
const TEXT = {
  zh: {
    title: "购物日志",
    subtitle: "// 购物记录小票",
    shoppingInfo: "购物信息 (Shopping Info)",
    items: "购物清单 (Items)",
    metrics: "消费评估 (Metrics)",
...
export default function ShoppingReceiptPage() {
  ...
  // 使用自定义 Hook 处理下载
  const { isDownloading, handleDownload: download, error } = useDownloadReceipt('SPENDING_LOG');
    ...
    <div className="fixed inset-0 flex flex-col overflow-hidden font-receipt tracking-receipt text-gray-900 bg-gray-50 md:flex-row">
      ...
      <div className={`w-full md:w-1/2 flex flex-col h-full bg-white md:border-r border-gray-200 z-10 transition-transform duration-300 ${mobileTab === 'edit' ? 'translate-x-0' : '-translate-x-full md:translate-x-0'} absolute md:relative`}>
        ...
        <div className="flex-1 p-4 pb-24 space-y-3 overflow-y-auto md:pb-10 no-scrollbar">
           ...
           <InputSection title={t.shoppingInfo} isOpen={expandedSection === 'shopping'} onToggle={() => toggleSection('shopping')}>
             ...
             <div className="grid grid-cols-2 gap-4">
               <div>
                 <label className="block text-[10px] font-bold text-gray-400 mb-1.5 uppercase">{t.labelChannel}</label>
                 <select value={data.channel} onChange={e => setData({...data, channel: e.target.value})} className="w-full bg-gray-50 border border-gray-200 p-2.5 text-sm focus:border-black outline-none rounded-sm">
                   {Object.entries(t.channelOptions).map(([key, label]) => <option key={key} value={key}>{label}</option>)}
                 </select>
               </div>
               <div>
                 <label className="block text-[10px] font-bold text-gray-400 mb-1.5 uppercase">{t.labelCategory}</label>
                 <select value={data.category} onChange={e => setData({...data, category: e.target.value})} className="w-full bg-gray-50 border border-gray-200 p-2.5 text-sm focus:border-black outline-none rounded-sm">
                   {Object.entries(t.categoryOptions).map(([key, label]) => <option key={key} value={key}>{label}</option>)}
                 </select>
               </div>
             </div>
           </InputSection>

           <InputSection title={t.items} isOpen={expandedSection === 'items'} onToggle={() => toggleSection('items')}>
             <div className="space-y-3">
               {data.items.map((item, index) => (
                 <div key={index} className="flex items-center gap-2">
                   ...
                   />
                   <button onClick={() => removeItem(index)} className="p-2 text-gray-400 transition-colors hover:text-red-500">
                     <Trash2 size={16} />
                   </button>
                 </div>
               ))}
               <button onClick={addItem} className="flex items-center justify-center w-full gap-2 py-2 text-sm font-bold text-gray-500 transition-colors border-2 border-gray-300 border-dashed hover:border-black hover:text-black">
           ...
           <InputSection title={t.metrics} isOpen={expandedSection === 'metrics'} onToggle={() => toggleSection('metrics')}>
             <div className="p-4 space-y-4 border border-gray-100 rounded-sm bg-gray-50">
               <RatingSlider label={t.labelNecessity} value={data.metrics.necessity} onChange={v => setData({...data, metrics: {...data.metrics, necessity: v}})} />
               <RatingSlider label={t.labelRegret} value={data.metrics.regret} onChange={v => setData({...data, metrics: {...data.metrics, regret: v}})} />
               <RatingSlider label={t.labelJoy} value={data.metrics.joy} onChange={v => setData({...data, metrics: {...data.metrics, joy: v}})} />
               <RatingSlider label={t.labelValue} value={data.metrics.value} onChange={v => setData({...data, metrics: {...data.metrics, value: v}})} />
             </div>
             <div>
               <label className="block text-[10px] font-bold text-gray-400 mb-1.5 uppercase">{t.labelVerdict}</label>
               <select value={data.verdict} onChange={e => setData({...data, verdict: e.target.value})} className="w-full bg-gray-50 border border-gray-200 p-2.5 text-sm focus:border-black outline-none rounded-sm">
                 {Object.entries(t.verdictOptions).map(([key, label]) => <option key={key} value={key}>{label}</option>)}
               </select>
             </div>
        ...
        <div className="w-full h-full overflow-y-auto no-scrollbar">
          <div className="flex items-center justify-center min-h-full py-20 md:py-10">
            <div className="transition-transform duration-500 origin-top scale-90 receipt-wrapper sm:scale-100">
              <ShoppingPreview data={data} text={t} total={total} hasTotal={hasTotal} ref={receiptRef} />
            </div>
          </div>
        </div>
      </div>
 ...
 * 展示购物清单、消费总额和消费评价
 */
const ShoppingPreview = React.forwardRef(({ data, text, total, hasTotal }, ref) => {
  const metricMap = {
    ...
    <div ref={ref} className="w-[380px] flex flex-col relative">
      <div className="flex flex-col flex-1 p-6 pb-8 font-receipt tracking-receipt text-black bg-white shadow-sm">
        ...
        <div className="grid grid-cols-2 mb-4 text-xs font-bold leading-relaxed uppercase gap-y-1">
          ...
          {data.location && data.location.trim() && <><div className="text-gray-500">{text.labelLocation}:</div><div className="text-right text-black truncate">{data.location}</div></>}
          {data.channel && <><div className="text-gray-500">{text.labelChannel}:</div><div className="text-right text-black">{text.channelOptions[data.channel]}</div></>}
          {data.category && <><div className="text-gray-500">{text.labelCategory}:</div><div className="text-right text-black">{text.categoryOptions[data.category]}</div></>}
        </div>
              ...
              {validItems.map((item, index) => (
                <div key={index} className="flex justify-between text-xs">
                  ...
                  {item.price !== null && item.price !== undefined && !isNaN(item.price) && (
                    <span className="font-bold">¥{item.price.toFixed(0)}</span>
                  )}
                </div>
              ))}
            </div>
        ...
        <div className="p-3 mb-4 text-center border-2 border-black">
          <div className="text-[9px] font-bold text-gray-500 uppercase mb-1">{text.labelVerdict}</div>
          <div className="text-lg font-black">{text.verdictOptions[data.verdict]}</div>
        </div>
