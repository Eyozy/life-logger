...
import { ChevronLeft, Download, Loader2, ChevronDown, Globe, Activity, Dumbbell, Flame, Eye } from 'lucide-react';
...
export default function FitnessReceiptPage() {
  ...
  const [data, setData] = useState({
    ...
    exercises: [ // New: Structured exercises
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      link.download = `BODY_LOG_${Date.now()}.png`;
      link.href = dataUrl;
      link.click();
    } catch (error) {
      alert("Fail");
    ...
    <div className="fixed inset-0 bg-gray-50 text-gray-900 font-mono flex flex-col md:flex-row overflow-hidden">
      ...
      <div className={`w-full md:w-1/2 flex flex-col h-full bg-white md:border-r border-gray-200 z-10 transition-transform duration-300 ${mobileTab === 'edit' ? 'translate-x-0' : '-translate-x-full md:translate-x-0'} absolute md:relative`}>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3 pb-24 md:pb-10 no-scrollbar">
           ...
           <InputSection title={t.workoutInfo} isOpen={expandedSection === 'workout'} onToggle={() => toggleSection('workout')}>
             <div>
               <label className="block text-[10px] font-bold text-gray-400 mb-1.5 uppercase">{t.labelType}</label>
               <select value={data.type} onChange={e => setData({...data, type: e.target.value})} className="w-full bg-gray-50 border border-gray-200 p-2.5 text-sm focus:border-black outline-none rounded-sm">
             ...
             <InputGroup label={t.labelLocation} value={data.location} onChange={v => setData({...data, location: v})} />
           </InputSection>
           ...
           <InputSection title={t.metrics} isOpen={expandedSection === 'metrics'} onToggle={() => toggleSection('metrics')}>
             <div className="space-y-4 bg-gray-50 p-4 rounded-sm border border-gray-100">
               <RatingSlider label={t.labelPump} value={data.metrics.pump} onChange={v => setData({...data, metrics: {...data.metrics, pump: v}})} />
               <RatingSlider label={t.labelPain} value={data.metrics.pain} onChange={v => setData({...data, metrics: {...data.metrics, pain: v}})} />
               <RatingSlider label={t.labelSweat} value={data.metrics.sweat} onChange={v => setData({...data, metrics: {...data.metrics, sweat: v}})} />
               <RatingSlider label={t.labelFocus} value={data.metrics.focus} onChange={v => setData({...data, metrics: {...data.metrics, focus: v}})} />
               <RatingSlider label={t.labelIntensity} value={data.intensity} onChange={v => setData({...data, intensity: parseInt(v)})} />
               <InputGroup label={t.labelCalories} value={data.caloriesBurned} onChange={v => setData({...data, caloriesBurned: v})} />
             </div>
           </InputSection>

           <InputSection title={t.log} isOpen={expandedSection === 'log'} onToggle={() => toggleSection('log')}>
             <InputGroup label={t.labelNotesSummary} value={data.notes} onChange={v => setData({...data, notes: v})} isTextArea />
           </InputSection>
      ...
      <nav className="md:hidden fixed bottom-6 left-1/2 -translate-x-1/2 z-50 bg-black text-white rounded-full px-1.5 py-1.5 shadow-2xl flex gap-1" role="tablist" aria-label={lang === 'zh' ? '视图切换' : 'View tabs'}>
        ...
        >
          <Dumbbell size={14} aria-hidden="true" /> {lang === 'zh' ? '编辑' : 'Edit'}
        </button>
        <button 
...
const MetricBar = ({ label, value }) => (
  <div className="flex items-center gap-2">
    <span className="w-24 text-[10px] font-bold uppercase text-right truncate">{label}</span>
    <div className="flex-1 h-3 border border-black p-[1px] bg-gray-100 relative">
      <div className="h-full bg-black relative z-10" style={{ width: `${value}%` }}></div>
    </div>
...
const FitnessPreview = React.forwardRef(({ data, text }, ref) => {
  ...
  const totalWorkoutVolume = data.exercises.reduce((sum, ex) => sum + (ex.sets * ex.reps * ex.weight), 0);
    ...
    <div ref={ref} className="w-[380px] flex flex-col relative">
      ...
      <div className="bg-white p-6 pb-8 flex-1 flex flex-col font-mono shadow-sm text-black">
        ...
        <div className="text-center mb-4">
          <h1 className="text-2xl font-black tracking-widest uppercase mb-1">{text.title}</h1>
        ...
        <div className="grid grid-cols-2 gap-y-1 text-xs font-bold uppercase mb-4 leading-relaxed">
          <div className="text-gray-500">{text.labelDate}:</div><div className="text-black text-right">{data.date}</div>
          <div className="text-gray-500">{text.labelTime}:</div><div className="text-black text-right">{data.time}</div>
          <div className="text-gray-500">{text.labelType}:</div><div className="text-black text-right">{text.types[data.type]}</div>
          <div className="text-gray-500">{text.labelLocation}:</div><div className="text-black text-right truncate">{data.location}</div>
        </div>
        ...
        <div className="border-y-2 border-dashed border-black py-4 mb-6">
          <div className="grid grid-cols-2 gap-y-4 gap-x-4">
              <div>
                 <div className="text-[9px] font-bold text-gray-500 uppercase mb-1">{text.labelDurationSummary}</div>
                 <div className="text-sm font-bold">{data.duration} MIN</div>
              </div>
              <div>
                 <div className="text-[9px] font-bold text-gray-500 uppercase mb-1">{text.labelCalories}</div>
                 <div className="text-sm font-bold">{data.caloriesBurned} KCAL</div>
              </div>
              <div className="col-span-2">
                 <div className="text-[9px] font-bold text-gray-500 uppercase mb-1">{text.labelIntensity}</div>
                 <div className="flex items-center gap-2">
                    <Bar val={data.intensity} />
                    <span className="w-8 text-right text-xs font-bold">{data.intensity}%</span>
                 </div>
              </div>
          </div>
        </div>
        ...
        {data.exercises && data.exercises.length > 0 && (
          <div className="mb-6">
            <div className="text-[10px] font-bold uppercase border-b border-black pb-1 mb-2">-- {text.labelExercises} --</div>
            <ul className="space-y-1.5 text-xs">
              {data.exercises.map((ex, idx) => (
                <li key={idx} className="flex justify-between items-start leading-tight">
                  <span className="max-w-[200px] truncate">{ex.name}</span>
                  <span className="text-right whitespace-nowrap font-bold">{ex.sets}x{ex.reps} @ {ex.weight}{ex.unit}</span>
                </li>
              ))}
            </ul>
            <div className="flex justify-between text-sm font-black border-t-2 border-black pt-2 mt-4">
              <span>TOTAL VOLUME:</span>
              <span>{totalWorkoutVolume} {data.exercises[0]?.unit || 'KG'}</span>
            </div>
          </div>
        )}
        ...
        <div className="mb-6 space-y-2">
           <div className="text-[10px] font-bold uppercase border-b border-black pb-1 mb-2">-- {text.labelBodyFeedback} --</div>
        ...
        {data.notes && data.notes.trim() && (
          <div className="mb-6 flex-1">
             <div className="text-[10px] font-bold uppercase border-b border-black pb-1 mb-2">{text.labelNotesSummary}</div>
             <p className="text-xs font-mono leading-relaxed whitespace-pre-wrap">{data.notes}</p>
          </div>
        )}
