...
export default function GameReceiptPage() {
  ...
  const [data, setData] = useState({
    ...
    completion: 78,
    memorableMoment:
      "第一次骑着马在宁姆格福平原上奔跑，看到夕阳下的黄金树，那种震撼感难以忘怀。",
    thoughts:
      "开放世界的巅峰之作，战斗系统深度十足，但对于新手玩家门槛较高。收集要素丰富，但有时候会让人感到压力。",
  });

  const [expandedSection, setExpandedSection] = useState("game");
  const receiptRef = useRef(null);
  const { isDownloading, handleDownload, error } = useDownloadReceipt('GAME_LOG');
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3 pb-24 md:pb-10 no-scrollbar">
          ...
          >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <InputGroup
                label={t.labelPlaytime}
                value={data.playtime}
                onChange={(v) => setData({ ...data, playtime: v })}
              />
              <InputGroup
                label={t.labelDate}
                value={data.date}
          ...
          <InputSection
            title={t.gameplay}
            isOpen={expandedSection === "gameplay"}
            onToggle={() => toggleSection("gameplay")}
          >
            <div className="space-y-4 bg-gray-50 p-4 rounded-sm border border-gray-100">
              <RatingSlider
                label={t.labelGraphics}
                value={data.gameplay.graphics}
                onChange={(v) =>
                  setData({
                    ...data,
                    gameplay: { ...data.gameplay, graphics: v },
                  })
                }
              />
              <RatingSlider
                label={t.labelGameplay}
                value={data.gameplay.gameplay}
                onChange={(v) =>
                  setData({
                    ...data,
                    gameplay: { ...data.gameplay, gameplay: v },
                  })
                }
              />
              <RatingSlider
                label={t.labelStory}
                value={data.gameplay.story}
                onChange={(v) =>
                  setData({ ...data, gameplay: { ...data.gameplay, story: v } })
                }
              />
              <RatingSlider
                label={t.labelDifficulty}
                value={data.gameplay.difficulty}
                onChange={(v) =>
                  setData({
                    ...data,
                    gameplay: { ...data.gameplay, difficulty: v },
                  })
                }
              />
              ...
              <div className="border-t border-gray-200 pt-4 mt-2">
                <RatingSlider
                  label={t.labelChallenge}
                  value={data.challenge}
                  onChange={(v) => setData({ ...data, challenge: v })}
                />
              </div>
              <RatingSlider
                label={t.labelAddiction}
                value={data.addiction}
                onChange={(v) => setData({ ...data, addiction: v })}
              />
            </div>
          </InputSection>
          ...
          >
            <InputGroup
              label={t.labelAchievement}
              value={data.achievement}
              onChange={(v) => setData({ ...data, achievement: v })}
            />
            <RatingSlider
              label={t.labelCompletion}
              value={data.completion}
              onChange={(v) => setData({ ...data, completion: v })}
            />
          </InputSection>
          ...
          >
            <InputGroup
              label={t.labelMemorable}
              value={data.memorableMoment}
              onChange={(v) => setData({ ...data, memorableMoment: v })}
              isTextArea
            />
            <InputGroup
    ...
    <div ref={ref} className="w-[380px] relative">
      <div className="min-h-[600px] bg-white p-6 pb-8 font-receipt tracking-receipt flex flex-col">
        ...
        <div className="text-center mb-4">
          <h1
            className="font-bold tracking-wider mb-1 text-black"
          ...
          <div className="mb-6">
            ...
            <div className="flex gap-2 text-[10px] font-bold uppercase mb-2">
              ...
              <span className="bg-black text-white px-2">
                {text.platforms[data.platform]}
              </span>
            </div>
            <p className="text-xs font-bold text-gray-600">
              {text.labelDeveloper}: {data.developer}
            </p>
          </div>
        )}
            ...
            <div className="flex-1 space-y-1">
              ...
              <div className="h-2 bg-gray-200 border border-black relative">
                <div
                  ...
                  style={{ width: `${data.gameplay.graphics}%` }}
                ></div>
              </div>
              <div className="flex justify-between text-[10px] font-bold uppercase">
              ...
              <div className="h-2 bg-gray-200 border border-black relative">
                <div
                  className="h-full bg-black"
                  style={{ width: `${data.gameplay.gameplay}%` }}
                ></div>
              </div>
              <div className="flex justify-between text-[10px] font-bold uppercase">
              ...
              <div className="h-2 bg-gray-200 border border-black relative">
                <div
                  className="h-full bg-black"
                  style={{ width: `${data.gameplay.story}%` }}
                ></div>
              </div>
              <div className="flex justify-between text-[10px] font-bold uppercase">
              ...
              <div className="h-2 bg-gray-200 border border-black relative">
                <div
                  className="h-full bg-black"
                  style={{ width: `${data.gameplay.difficulty}%` }}
                ></div>
              </div>
            </div>
          </div>
        ...
        {data.memorableMoment && data.memorableMoment.trim() && (
          <div className="mb-6">
            ...
            <div className="relative p-4 bg-gray-50 border-l-4 border-black">
              <p className="text-sm italic leading-relaxed text-gray-800 text-justify whitespace-pre-wrap">
        ...
        {data.thoughts && data.thoughts.trim() && (
          <div className="mb-6 flex-1">
            ...
            <p className="text-sm leading-relaxed border-b border-gray-300 pb-2 whitespace-pre-wrap">
              {data.thoughts}
            </p>
          </div>
        )}
