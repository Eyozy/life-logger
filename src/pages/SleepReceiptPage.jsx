        ...
        <div className="flex-1 p-4 pb-24 space-y-3 overflow-y-auto md:pb-10 no-scrollbar">
          ...
          >
            ...
            <div className="grid grid-cols-2 gap-4">
              <InputGroup
                label={t.labelBedtime}
                value={data.bedtime}
                onChange={(v) => setData({ ...data, bedtime: v })}
              />
              <InputGroup
                label={t.labelWakeTime}
                value={data.wakeTime}
                onChange={(v) => setData({ ...data, wakeTime: v })}
              />
            </div>
            <div>
          ...
          >
            <div className="p-4 space-y-4 border border-gray-100 rounded-sm bg-gray-50">
              <RatingSlider
                ...
                value={data.metrics.quality}
                onChange={(v) =>
                  setData({ ...data, metrics: { ...data.metrics, quality: v } })
                }
              />
              <RatingSlider
                label={t.labelDreamClarity}
                value={data.metrics.dreamClarity}
                onChange={(v) =>
                  setData({
                    ...data,
                    metrics: { ...data.metrics, dreamClarity: v },
                  })
                }
              />
              <RatingSlider
                label={t.labelWakeFreshness}
                value={data.metrics.wakeFreshness}
                onChange={(v) =>
                  setData({
                    ...data,
                    metrics: { ...data.metrics, wakeFreshness: v },
                  })
                }
              />
              <RatingSlider
                label={t.labelDeepSleep}
                value={data.metrics.deepSleep}
                onChange={(v) =>
                  setData({
                    ...data,
                    metrics: { ...data.metrics, deepSleep: v },
                  })
                }
              />
            </div>
          ...
          <InputSection
            title={t.preSleepSection}
            isOpen={expandedSection === "presleep"}
            onToggle={() => toggleSection("presleep")}
          >
                  ...
                  onClick={() =>
                    setData({
                      ...
                      preSleepActivities: {
                        ...
                        [key]: !data.preSleepActivities[key],
                      },
                    })
                  }
                  className={`p-2 text-xs font-bold border-2 rounded transition-all ${
                    data.preSleepActivities[key]
          ...
          <InputSection
            title={t.dreamLog}
            isOpen={expandedSection === "dream"}
            onToggle={() => toggleSection("dream")}
          >
            <InputGroup
              label={t.labelDreamContent}
              value={data.dreamContent}
              onChange={(v) => setData({ ...data, dreamContent: v })}
              isTextArea
            />
            <InputGroup
              label={t.labelNotes}
    ...
    <div ref={ref} className="w-[380px] flex flex-col relative">
      <div className="flex flex-col flex-1 p-6 pb-8 font-receipt tracking-receipt text-black bg-white shadow-sm">
        ...
        <div className="grid grid-cols-2 mb-4 text-xs font-bold leading-relaxed uppercase gap-y-1">
          ...
          {data.bedtime && (
            <>
              <div className="text-gray-500">{text.labelBedtime}:</div>
              <div className="text-right text-black">{data.bedtime}</div>
            </>
          )}
          {data.wakeTime && (
            <>
              <div className="text-gray-500">{text.labelWakeTime}:</div>
              <div className="text-right text-black">{data.wakeTime}</div>
            </>
          )}
          {duration && (
            <>
              <div className="text-gray-500">{text.labelDuration}:</div>
              <div className="font-black text-right text-black">{duration}</div>
            </>
          )}
        </div>
        ...
        <div className="py-4 mb-6 text-center border-black border-dashed border-y-2">
          <div className="text-[9px] font-bold text-gray-500 uppercase mb-1">
            {text.labelMorningMood}
          </div>
          <div className="text-lg font-black">
            {text.moodOptions[data.morningMood]}
          </div>
        </div>
