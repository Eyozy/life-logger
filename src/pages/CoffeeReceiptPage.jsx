        ...
        <div className="flex-1 p-4 pb-24 space-y-3 overflow-y-auto md:pb-10 no-scrollbar">
          ...
          >
            ...
            <InputGroup
              label={t.labelShop}
              value={data.shop}
              onChange={(v) => setData({ ...data, shop: v })}
            />
            <InputGroup
              label={t.labelDrink}
              value={data.drink}
              onChange={(v) => setData({ ...data, drink: v })}
            />
            <div className="grid grid-cols-2 gap-4">
              <div>
            ...
            <InputGroup
              label={t.labelBean}
              value={data.bean}
              onChange={(v) => setData({ ...data, bean: v })}
            />
            <div className="grid grid-cols-2 gap-4">
              <InputGroup
                label={t.labelCaffeine}
                value={data.caffeine}
                onChange={(v) =>
                  setData({ ...data, caffeine: parseInt(v) || 0 })
                }
                type="number"
              />
              <InputGroup
                label={t.labelTodayTotal}
                value={data.todayTotal}
                onChange={(v) =>
                  setData({ ...data, todayTotal: parseInt(v) || 0 })
                }
                type="number"
              />
            </div>
          ...
          >
            <div className="p-4 space-y-4 border border-gray-100 rounded-sm bg-gray-50">
              <RatingSlider
                label={t.labelBitterness}
                value={data.metrics.bitterness}
                onChange={(v) =>
                  setData({
                    ...data,
                    metrics: { ...data.metrics, bitterness: v },
                  })
                }
              />
              <RatingSlider
                label={t.labelAcidity}
                value={data.metrics.acidity}
                onChange={(v) =>
                  setData({ ...data, metrics: { ...data.metrics, acidity: v } })
                }
              />
              <RatingSlider
                label={t.labelSweetness}
                value={data.metrics.sweetness}
                onChange={(v) =>
                  setData({
                    ...data,
                    metrics: { ...data.metrics, sweetness: v },
                  })
                }
              />
              <RatingSlider
                ...
                onChange={(v) =>
                  setData({
                    ...data,
                    metrics: { ...data.metrics, satisfaction: v },
                  })
                }
              />
            </div>
        ...
        <div className="grid grid-cols-2 mb-4 text-xs font-bold leading-relaxed uppercase gap-y-1">
          ...
          {data.time && (
            <>
              ...
              <div className="text-right text-black">{data.time}</div>
            </>
          )}
          {data.shop && data.shop.trim() && (
            <>
              <div className="text-gray-500">{text.labelShop}:</div>
              <div className="text-right text-black truncate">{data.shop}</div>
            </>
          )}
          {data.drink && data.drink.trim() && (
            <>
              <div className="text-gray-500">{text.labelDrink}:</div>
              <div className="text-right text-black">{data.drink}</div>
            </>
          )}
          {data.size && (
            <>
              <div className="text-gray-500">{text.labelSize}:</div>
              <div className="text-right text-black">
                {text.sizeOptions[data.size]}
              </div>
            </>
          )}
          {data.method && (
            <>
              <div className="text-gray-500">{text.labelMethod}:</div>
              <div className="text-right text-black">
                {text.methodOptions[data.method]}
              </div>
            </>
          )}
        </div>
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="text-right">
                ...
                <div className="text-sm font-bold">
                  [{caffeineInfo.icon}] {caffeineInfo.status}
                </div>
              </div>
            </div>
          </div>
