...
import { ChevronLeft, Download, Loader2, ChevronDown, Globe, Users, Battery, BatteryWarning, Zap, Eye, CheckSquare, Square } from 'lucide-react';

const TEXT = {
  zh: {
    title: "社交观察",
    subtitle: "// 社交日志小票",
    context: "交互情境 (Context)",
    metrics: "状态诊断 (Diagnosis)",
    analysis: "详细分析 (Analysis)",
...
export default function SocialReceiptPage() {
  ...
  const [data, setData] = useState({
    ...
    awkward: 85,
    vibe: 30,
    summary: "全程都在聊他的加密货币投资，完全没有问过我的情况。咖啡很难喝，气氛很尴尬。我想回家打游戏。",
    verdict: "RECHARGE"
  });
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      link.download = `SOCIAL_LOG_${Date.now()}.png`;
      link.href = dataUrl;
      link.click();
    } catch (error) {
      alert("Fail");
      ...
      <div className={`w-full md:w-1/2 flex flex-col h-full bg-white md:border-r border-gray-200 z-10 transition-transform duration-300 ${mobileTab === 'edit' ? 'translate-x-0' : '-translate-x-full md:translate-x-0'} absolute md:relative`}>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3 pb-24 md:pb-10 no-scrollbar">
           ...
           <InputSection title={t.context} isOpen={expandedSection === 'context'} onToggle={() => toggleSection('context')}>
             <InputGroup label={t.labelSubject} value={data.subject} onChange={v => setData({...data, subject: v})} />
           ...
           <InputSection title={t.metrics} isOpen={expandedSection === 'metrics'} onToggle={() => toggleSection('metrics')}>
             <div className="space-y-4 bg-gray-50 p-4 rounded-sm border border-gray-100">
               <SliderItem label={t.labelBattery} value={data.battery} onChange={v => setData({...data, battery: v})} icon={Battery} />
               <SliderItem label={t.labelAwkward} value={data.awkward} onChange={v => setData({...data, awkward: v})} icon={BatteryWarning} />
               <SliderItem label={t.labelVibe} value={data.vibe} onChange={v => setData({...data, vibe: v})} icon={Zap} />
             </div>
           </InputSection>

           <InputSection title={t.analysis} isOpen={expandedSection === 'analysis'} onToggle={() => toggleSection('analysis')}>
             <InputGroup label={t.labelSummary} value={data.summary} onChange={v => setData({...data, summary: v})} isTextArea />
             <div>
               ...
               <div className="grid grid-cols-2 gap-2">
                 {Object.entries(t.verdicts).map(([key, label]) => (
                   <button
                     ...
                     className={`p-2 text-xs border rounded-sm text-left flex items-center gap-2 transition-all ${data.verdict === key ? 'bg-black text-white border-black' : 'bg-white border-gray-200 text-gray-600 hover:border-gray-300'}`}
                   >
                     {data.verdict === key ? <CheckSquare size={14} /> : <Square size={14} />}
                     {label}
                   </button>
                 ))}
               </div>
  ...
  return (
    <div ref={ref} className="w-[380px] relative">
      ...
      <div className="text-center mb-4">
        <h1 className="text-3xl font-bold tracking-wider mb-1 text-black">{text.labelHeader}</h1>
      ...
      {data.subject && data.subject.trim() && (
        <div className="mb-6">
          <div className="grid grid-cols-2 gap-y-1 text-sm mb-4">
            <div className="text-gray-500 font-bold">{text.labelSubject}:</div>
            <div className="text-right truncate">{data.subject}</div>
            <div className="text-gray-500 font-bold">{text.labelType}:</div>
            <div className="text-right truncate">{text.types[data.type]}</div>
            <div className="text-gray-500 font-bold">{text.labelLocation}:</div>
            <div className="text-right truncate">{data.location}</div>
            <div className="text-gray-500 font-bold">{text.labelDuration}:</div>
      ...
      <div className="mb-4">
        ...
        <div className="h-2 bg-gray-200 border border-black relative">
          <div className="h-full bg-black transition-all" style={{ width: `${data.battery}%` }}></div>
        </div>
      </div>
      ...
      <div className="mb-6 space-y-4">
        <div>
          ...
          <div className="h-2 bg-gray-200 border border-black relative">
            <div className="h-full bg-black transition-all" style={{ width: `${data.awkward}%` }}></div>
          </div>
        </div>
        <div>
          <div className="flex justify-between text-xs font-bold uppercase mb-1">
          ...
          <div className="h-2 bg-gray-200 border border-black relative">
            <div className="h-full bg-black transition-all" style={{ width: `${data.vibe}%` }}></div>
          </div>
        </div>
      </div>
      ...
      {data.summary && data.summary.trim() && (
        <div className="mb-6">
          <div className="text-[10px] font-bold uppercase text-gray-500 border-b border-black pb-1 mb-2">{text.labelSummary}</div>
      ...
      {/* Verdict - 2x2 Grid */}
      <div className="mb-6">
        <div className="text-[10px] font-bold uppercase text-gray-500 border-b border-black pb-1 mb-2">{text.labelVerdict}</div>
      ...
      <div className="mt-auto pt-6 flex flex-col items-center">
        <div className="text-[10px] font-bold uppercase tracking-widest mb-2 text-center">End of Social Analysis Report</div>
